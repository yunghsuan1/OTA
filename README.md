...
### 3. 實作 App 3 互動功能 (已完成程式碼)
* **成果**：已於 2026-05-13 完成 App 3 的程式碼撰寫與 Python UI 更新。
* **功能**：
  * 網路連線成功（取得 IP）後，才開始閃爍三燈跑馬燈。
  * PC 端 UI 新增 `QUERY INFO` 按鈕，點擊後發送 `"INFO"`。
  * 板子收到 `"INFO"` 指令後，回傳 `"App 3 (Marquee + Info) is running!\n"`。

---

## 🔮 未來展望 (Future Roadmap)

為了讓 OTA 系統更加完善，後續預計加入以下功能：

### 1. 軟體觸發 Fallback (免按鍵切換)
* **目標**：在不依賴外部 S1 按鍵的情況下，透過網路指令讓板子從 App 2/3 切換回 App 1。
* **實作方法 (Data Flash 旗標)**：將「強制啟動 App 1」的標記寫入 Data Flash。MCUboot 開機時讀取該旗標，決定是否跳過版本檢查直接執行 Slot 1。
